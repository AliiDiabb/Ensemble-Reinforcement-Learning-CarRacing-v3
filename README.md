...
* **Environment:** `CarRacing-v3` (Gymnasium)[cite: 3]
* **Framework:** Stable-Baselines3 (PPO Algorithm)[cite: 3]
* **Monitoring:** TensorBoard logging for rewards, losses, and evaluation metrics[cite: 3]

### 2. Hyperparameter Configurations
Three distinct PPO agents were trained for **300,000 timesteps** each[cite: 3]:
* **Agent 1:** $\text{Learning Rate} = 3 \times 10^{-4}$ (Standard parameters)[cite: 3]
* **Agent 2:** $\text{Learning Rate} = 1 \times 10^{-5}$ (Increased epochs for stability)[cite: 3]
* **Agent 3:** $\text{Learning Rate} = 1 \times 10^{-3}$ (Testing rapid adaptation)[cite: 3]

### 3. Ensemble Strategy
1. Each trained agent predicts an action independently based on the current environment state[cite: 3].
2. Actions are aggregated across agents using a **majority voting mechanism**[cite: 3].
3. The consensus action is executed in the environment[cite: 3].
